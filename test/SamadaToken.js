...
describe("Smada Token Contract", function () {
  ...
  describe("Should Transfer correctly", () => {
    it("Should fail when not having enough tokens", async () => {
      const { contract, owner, address1 } = await loadFixture(deploySamadaTokenFixture)
      const initialOwnerBalance = await contract.balanceOf(owner.address)
      await expect(contract.connect(address1).transfer(owner.address, 5)).to.be.revertedWith("ERC20: transfer amount exceeds balance")
